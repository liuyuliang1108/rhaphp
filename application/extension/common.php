...
function write_config($config)
{
    if (is_array($config)) {
        //读取配置内容
        $conf = file_get_contents(ROOT_PATH .config('install_dir')[1][3]. '/'.config('install_data')['db']);
        //替换配置项
...
{
    //读取SQL文件
    if(!is_file(ROOT_PATH . config('install_dir')[1][3]. '/'.config('install_data')['sql'])){
        show_msg(ROOT_PATH . config('install_dir')[1][3]. '/'.config('install_data')['sql'].' 安装数据库文件不存在');
        exit;
    }
    $sql = file_get_contents(ROOT_PATH . config('install_dir')[1][3]. '/'.config('install_data')['sql']);
    $sql = str_replace("\r", "\n", $sql);
    $sql = explode(";\n", $sql);
